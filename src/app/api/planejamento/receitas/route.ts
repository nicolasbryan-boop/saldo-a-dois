...
const schema = z.object({
  name: z.string().trim().min(1, 'Dê um nome para a receita').max(80),
...
export const GET = handle(async () => {
  ...
  return jsonOk({ items: await listIncomeSources(context.db, context.household.id) });
});
...
export const POST = handle(async (request) => {
  ...
  const item = await createIncomeSource(
    context.db,
